...
def align_variable_numpy(X,maxL,paddingNumber): # X is bacth==several sentences

    aligned = []
    for cur in X: # cur==one sentence
        ext_cur = []
        ext_cur.extend(cur)
...
def sample_a_sorted_batch_from_numpy(numpyX,numpyY,batch_size,use_cuda):
    if batch_size != None:
        select_index = random.sample(range(len(numpyY)), batch_size) # select batch_size digits from range
    else:
    ...
    index_decoder_X,index_decoder_Y = get_decoder_index_XY(batch_y) # returnX & returnY

    all_lens = np.array([len(x) for x in batch_y])
    maxL = np.max(all_lens)

    idx = np.argsort(all_lens) # Returns the indices that would sort an array.
